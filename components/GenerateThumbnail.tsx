...
const GenerateThumbnail = ({
  ...
  const { startUpload } = useUploadFiles(generateUploadUrl);
	...
	const handleGenerateThumbnail = useAction(api.openai.generateImageThumbnailAction)

  const handleImage = async (blob:Blob ,fileName:string) => {
    setIsImageLoading(true)
    setImage('')
    try {
      //todo: finish up  handleImage 03:06:00
	...
	const generateImage = async () => {
		...
		try {
			...
			console.log(error)
			toast({
				title:'Error generation thumbnail',
				variant:'destructive'
			})
			setIsImageLoading(false)
		}
		...
		<>
			...
			{isAiThumbnail ? (
				...
				<div
					...
					onKeyUp={() => imageRef.current?.click()}
				>
					<Input
						type="file"
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-12 font-normal text-gray-1">SVG , PNG , OR GIF</p>
          </div>
				</div>
			)}
      {image &&
